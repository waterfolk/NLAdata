...
# eval_name
sites2012_trim0<-sites2012 %>% select(site_id,uid,date=date_col,visit_no,area_ha,elevation,lat=lat_dd83,lon=lon_dd83,manmade_natural=lake_origin,state,ag_eco3=aggr_eco3_2015,epa_reg,region=fw_eco9,cntyname)
sites2012_trim0<-sites2012_trim0 %>% arrange(site_id,date) %>% as.data.frame()
sites2012_trim0$year<-"2012"

sites2017_trim0<-sites2017 %>% select(site_id,uid,date=date_col,visit_no,area_ha,elevation,lat=lat_dd83,lon=lon_dd83,manmade_natural=lake_orgn,state=pstl_code,ag_eco3,epa_reg,region=ag_eco9,cntyname)
...
nla2012_secchi_08232016$var<-"Secchi"
...
for (i in 1:length(datakey[,1])){
  print(i)
  datakeyi<-datakey[i,]
  varcolsi<-strsplit(datakeyi$varcols[[1]],",")[[1]]
  #  numvarcolsi<-length(varcolsi)
  resultcolsi<-strsplit(datakeyi$resultcols[[1]],",")[[1]]
  #  numresultcolsi<-length(resultcolsi)
  refcolsi<-strsplit(datakeyi$refcols[[1]],",")[[1]]
  #  numrefcolsi<-length(refcolsi)
  othercolsi<-strsplit(datakeyi$othercols[[1]],",")[[1]]
  unitcolsi<-strsplit(datakeyi$unitcols[[1]],",")[[1]]
  numunitcolsi<-length(unitcolsi)
  obji<-datakeyi$obj
  datai<-get(obji)
  if(unitcolsi %in% names(datai) == FALSE){
    datai$units<-""
  }
  if("depth" %in% names(datai) == FALSE){
    datai$depth<-""
  }
  datai$year<-datakeyi$year
  datai$file<-obji
  if(length(unitcolsi)==0){unitcolsi="units"}
  if(length(resultcolsi)==1){
    dataformati<-datai %>% select(year,file,refcolsi,othercolsi,unitcolsi,varcolsi,depth,resultcolsi)  
  }
  if(length(resultcolsi)>1){
    dataformati<-datai %>% select(year,file,refcolsi,othercolsi,unitcolsi,varcolsi,depth,resultcolsi) %>%
      pivot_longer(cols=resultcolsi)#,unitcolsi)
    #  dataformati<-datai %>% select(refcolsi,othercolsi,unitcolsi,varcolsi,resultcolsi) %>%
    #    pivot_longer(cols=resultcolsi)
  }
  names(dataformati)<-c("year","file","uid","site_id","date_col","unit","depth","var","result")
...
which<-which(longdata$file %in% c("nla_2017_profile_data","nla2012_wide_profile_08232016") & longdata$var=="temperature" & longdata$result>50)
#longdata[which,]$result<-NA

##############################################################

longdata$file[grep("profile",longdata$file)] %>% unique()

profiles_top2m<-longdata %>% filter(site_id!="") %>% 
  filter(depth<=2, file %in% c("nla_2017_profile_data","nla2012_wide_profile_08232016")) %>% 
  ...
  dplyr::summarize(temperature=mean(value[which(var=="temperature")],na.rm=TRUE),
                   ...
                   conductivity=mean(value[which(var=="conductivity")],na.rm=TRUE)) %>% mutate(depth="top2m") %>% melt(id.vars=c("site_id","uid","date_col","year","depth"))


profiles_2to4m<-longdata %>% filter(site_id!="") %>% 
  filter(depth>2 & depth<=4, file %in% c("nla_2017_profile_data","nla2012_wide_profile_08232016")) %>% 
  group_by(site_id,uid,date_col,year) %>%
  dplyr::summarize(temperature=mean(value[which(var=="temperature")],na.rm=TRUE),
                   oxygen=mean(value[which(var=="oxygen")],na.rm=TRUE),
                   ph=mean(value[which(var=="ph")],na.rm=TRUE),
...
profiles_below4m<-longdata %>% filter(site_id!="") %>% 
  filter(depth>4, file %in% c("nla_2017_profile_data","nla2012_wide_profile_08232016")) %>% 
  group_by(site_id,uid,date_col,year) %>%
  dplyr::summarize(temperature=mean(value[which(var=="temperature")],na.rm=TRUE),
                   oxygen=mean(value[which(var=="oxygen")],na.rm=TRUE),
                   ph=mean(value[which(var=="ph")],na.rm=TRUE),
...
profiles_below5m<-longdata %>% filter(site_id!="") %>% 
  filter(depth>5, file %in% c("nla_2017_profile_data","nla2012_wide_profile_08232016")) %>% 
  group_by(site_id,uid,date_col,year) %>%
  dplyr::summarize(temperature=mean(value[which(var=="temperature")],na.rm=TRUE),
                   oxygen=mean(value[which(var=="oxygen")],na.rm=TRUE),
                   ph=mean(value[which(var=="ph")],na.rm=TRUE),
...
profiles_groups<-rbind(profiles_top2m,profiles_2to4m,profiles_below4m,profiles_below5m)
...
watershed<-watershed %>% select(site_id,depth,var,value) %>% arrange(site_id,var)
watershed$var<-as.character(watershed$var)
...
daterr1_null$value<-NA
daterr1_null$var<-NA
daterr1_null$depth<-NA
daterr1_null$uid<-NA
daterr1_null$file<-NA
daterr1_null$unit<-NA
daterr1_null$date_col<-NA
...
daterr_profiles_groups<-merge(daterr1_null,profiles_groups,by=c("site_id","year"))
daterr_profiles_groups<- daterr_profiles_groups %>% rename(uid=uid.y,date_col=date_col.y,var=var.y,value=value.y)
daterr_profiles_groups<- daterr_profiles_groups %>% select(-uid.x,-date_col.x,-var.x,-value.x)
...
daterr1<-rbind(daterr1,watershed)
...
#daterr1<-merge(daterr1,units,by="var",all=TRUE)

daterr1$unit[which(daterr1$var=="area_ha")]<-"ha"
daterr1$unit[which(daterr1$var=="chla")]<-"ug/L"
daterr1$unit[which(daterr1$var=="damcnt_bsn")]<-"number"
daterr1$unit[which(daterr1$var=="damden_bsn")]<-"number/km2"
daterr1$unit[which(daterr1$var=="depthmax")]<-"m"
daterr1$unit[which(daterr1$var=="elevation")]<-"m"
daterr1$unit[which(daterr1$var=="elevmax_bsn")]<-"m"
daterr1$unit[which(daterr1$var=="elevmean_bsn")]<-"m"
daterr1$unit[which(daterr1$var=="houseden_1000")]<-"units/mile2"
daterr1$unit[which(daterr1$var=="houseden_bsn")]<-"units/mile2"
daterr1$unit[which(daterr1$var=="latitude ")]<-"degrees N"
daterr1$unit[which(daterr1$var=="micl_result")]<-"ug/L"
daterr1$unit[which(daterr1$var=="micx_result")]<-"ug/L"
#daterr1$units[which(daterr1$var=="micl_result")]<-"NA"
#daterr1$units[which(daterr1$var=="micx_result")]<-"NA"

daterr1$unit[which(daterr1$var=="nadp_nh4_bsn")]<-"kg/km2"
daterr1$unit[which(daterr1$var=="nadp_no3_bsn")]<-"kg/km2"
daterr1$unit[which(daterr1$var=="nadp_totaln_bsn")]<-"kg/km2"
daterr1$unit[grep("pct_bsn",daterr1$var)]<-"%"
daterr1$unit[which(daterr1$var=="oxygentop2m")]<-"mg/L"
daterr1$unit[which(daterr1$var=="oxygen2to4m")]<-"mg/L"
daterr1$unit[which(daterr1$var=="oxygenbelow4m")]<-"mg/L"
daterr1$unit[which(daterr1$var=="oxygenbelow5m")]<-"mg/L"
daterr1$unit[which(daterr1$var=="ph")]<-"ph"
daterr1$unit[which(daterr1$var=="phtop2m")]<-"ph units"
daterr1$unit[which(daterr1$var=="ph2to4m")]<-"ph units"
daterr1$unit[which(daterr1$var=="phbelow4m")]<-"ph units"
daterr1$unit[which(daterr1$var=="phbelow5m")]<-"ph units"
daterr1$unit[which(daterr1$var=="secchi")]<-"m"
daterr1$unit[which(daterr1$var=="temperaturetop2m")]<-"C"
daterr1$unit[which(daterr1$var=="temperature2to4m")]<-"C"
daterr1$unit[which(daterr1$var=="temperaturebelow4m")]<-"C"
daterr1$unit[which(daterr1$var=="temperaturebelow5m")]<-"C"

daterr1$unit[grep("pind",daterr1$var)]<-"% of individuals present"
daterr1$unit[grep("ptax",daterr1$var)]<-"% of species present"
daterr1$unit[grep("pbio",daterr1$var)]<-"% of biomass"
daterr1$unit[grep("_den",daterr1$var)]<-"individuals per Liter"
daterr1$unit[grep("_bio",daterr1$var)]<-"biomass units"

daterr1$unit[grep("_phyto_density",daterr1$var)]<-"individuals per Liter"
daterr1$unit[grep("_zoop_density",daterr1$var)]<-"individuals per Liter"
daterr1$unit[grep("_phyto_biovolume",daterr1$var)]<-"biomass per Liter"

daterr1$unit[grep("_ntax",daterr1$var)]<-"number of taxa present"
daterr1$unit[grep("_ptax",daterr1$var)]<-"% of taxa present"



daterr1$var<-paste(daterr1$var," (",daterr1$unit,")",sep="")
...
daterr_profiles_start<-daterr[grep("profile",daterr$file),]
