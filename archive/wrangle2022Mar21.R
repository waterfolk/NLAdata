...
for (i in 1:length(datakey[,1])){
  ...
  obji<-datakeyi$obj
  datai<-get(obji)
  datai<-datai %>% mutate(units=unitcolsi)
...
#  }
  ...
  if(length(resultcolsi)==1){
    dataformati<-datai %>% select(year,file,refcolsi,othercolsi,units,varcolsi,depth,resultcolsi)  
  }
  if(length(resultcolsi)>1){
    dataformati<-datai %>% select(year,file,refcolsi,othercolsi,units,varcolsi,depth,resultcolsi) %>%
      pivot_longer(cols=resultcolsi,names_to="var")#,unitcolsi)
    
    dataformati<-dataformati %>% select(year,file,uid,site_id,date_col,units,var,depth,value)
    
    #  dataformati<-datai %>% select(refcolsi,othercolsi,unitcolsi,varcolsi,resultcolsi) %>%
    #    pivot_longer(cols=resultcolsi)
  }
  names(dataformati)<-c("year","file","uid","site_id","date_col","unit","var","depth","result")
