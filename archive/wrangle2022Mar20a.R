...
nla_2017_secchi_data$variable<-"Secchi depth"
...
nla2012_atrazine_08192016$variable<-"Atrazine"
nla2012_secchi_08232016$variable<-"Secchi"
...
datakey <- tribble(
  ...
  "units", "uid", "site_id,date_col",
  "nla2012_atrazine_08192016","2012","variable","atrazine_result",
  ...
  "units", "uid", "site_id,date_col",
  "nla2012_secchi_08232016","2012","variable","secchi",
  "units", "uid", "site_id,date_col",
  ...
  "units","uid","site_id,date_col",
  #  "nla_2017_benthic_count_data","2017","variable","result",
  #      "units","uid","site_id,date_col",
...
longdata<-longdata %>% rename(variable=var)
#as.numeric(longdata$result) %>% sort() %>% tail()


longdata$variable<-tolower(longdata$variable)
longdata$variable<-gsub("\\/","_",longdata$variable)
longdata$variable<-gsub("\\-","_",longdata$variable)
longdata$variable<-gsub("\\(","",longdata$variable)
longdata$variable<-gsub("\\)","",longdata$variable)
longdata$variable<-gsub("_result","",longdata$variable)

df<-data.frame(variable=longdata$variable,file=longdata$file) %>% unique() %>% arrange(variable)

# data entry error? delete these?
nla_2017_water_chemistry_chla_data %>% filter(analyte=="BATCH_ID")

which<-which(longdata$file %in% c("nla_2017_profile_data","nla2012_wide_profile_08232016") & longdata$variable=="temperature" & longdata$result>50)
longdata[which,]

############################
#"","variable","site_id","uid","date","name","lat","lon","manmade_natural","lake_origin12","state","aggr_eco3_2015","aggr_eco9_2015","epa_reg","regionL1","region","eco_bio","cntyname","depth","value","area_group","depth_group","units","group"
#"","site_id","uid","date","name","lat","lon","manmade_natural","lake_origin12","state","aggr_eco3_2015","aggr_eco9_2015","epa_reg","regionL1","region","eco_bio","cntyname","area_group","depth_group","group","depth","variable","value","namestate"
#############################
...
profiles_top2m<-longdata %>% 
  ...
  dplyr::summarize(temperature=mean(value[which(variable=="temperature")],na.rm=TRUE),
                   ...
                   ph=mean(value[which(variable=="ph")],na.rm=TRUE),
                   conductivity=mean(value[which(variable=="conductivity")],na.rm=TRUE)) %>% mutate(depth="top2m") %>% melt(id.vars=c("uid","depth"))

profiles_2to4m<-longdata %>% 
  filter(depth>2 & depth<=4, file %in% c("nla_2017_profile_data","nla2012_wide_profile_08232016")) %>% 
  group_by(uid) %>%
  dplyr::summarize(temperature=mean(value[which(variable=="temperature")],na.rm=TRUE),
                   oxygen=mean(value[which(variable=="oxygen")],na.rm=TRUE),
                   ph=mean(value[which(variable=="ph")],na.rm=TRUE),
                   conductivity=mean(value[which(variable=="conductivity")],na.rm=TRUE)) %>% mutate(depth="2to4m") %>% melt(id.vars=c("uid","depth"))

profiles_below4m<-longdata %>% 
  filter(depth>4, file %in% c("nla_2017_profile_data","nla2012_wide_profile_08232016")) %>% 
  group_by(uid) %>%
  dplyr::summarize(temperature=mean(value[which(variable=="temperature")],na.rm=TRUE),
                   oxygen=mean(value[which(variable=="oxygen")],na.rm=TRUE),
                   ph=mean(value[which(variable=="ph")],na.rm=TRUE),
                   conductivity=mean(value[which(variable=="conductivity")],na.rm=TRUE)) %>% mutate(depth="below4m") %>% melt(id.vars=c("uid","depth"))

profiles_below5m<-longdata %>% 
  filter(depth>5, file %in% c("nla_2017_profile_data","nla2012_wide_profile_08232016")) %>% 
  group_by(uid) %>%
  dplyr::summarize(temperature=mean(value[which(variable=="temperature")],na.rm=TRUE),
                   oxygen=mean(value[which(variable=="oxygen")],na.rm=TRUE),
                   ph=mean(value[which(variable=="ph")],na.rm=TRUE),
                   conductivity=mean(value[which(variable=="conductivity")],na.rm=TRUE)) %>% mutate(depth="below5m") %>% melt(id.vars=c("uid","depth"))
...
daterr_maxdepths$variable<-"depthmax"
