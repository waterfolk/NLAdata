...
profiles_top1m<-longdata %>% filter(site_id!="") %>%
  filter(depth<=1, file %in% c("nla_2017_profile_data","nla2012_wide_profile_08232016")) %>%
...
profiles_1to2m<-longdata %>% filter(site_id!="") %>%
  filter(depth>1 & depth<=2, file %in% c("nla_2017_profile_data","nla2012_wide_profile_08232016")) %>%
  group_by(site_id,uid,date_col,year) %>%
  dplyr::summarize(temperature=mean(value[which(var=="temperature")],na.rm=TRUE),
                   oxygen=mean(value[which(var=="oxygen")],na.rm=TRUE),
                   ph=mean(value[which(var=="ph")],na.rm=TRUE),
...
landnames2017<-names(nla2017_landMets)
landnames2012<-names(nla2012_wide_watershed)
#write.csv(landnames2017,"landnames2017.txt")
#write.csv(landnames2012,"landnames2012.txt")

landnames_df <- tribble(
  ~name, ~name2012, ~name2017,
  "damdens","damden_bsn","damdens",
  "nlcd_2006_crop_pct","nlcd2006_agricpct_bsn","pctcrop2006",
  "nlcd_2006_urbhi_pct","nlcd2006_24pct_bsn","pcturbhi2006",
  "nlcd_2006_urbmd_pct","nlcd2006_23pct_bsn","pcturbmd2006",
  "nlcd_2006_urblo_pct","nlcd2006_22pct_bsn","pcturblo2006",
  "nlcd_2006_wetlh_pct","nlcd2006_95pct_bsn","pcthbwet2006",
  "nlcd_2006_wetlw_pct","nlcd2006_90pct_bsn","pctwdwet2006",
  "nlcd_2006_water_pct","waterpct_bsn","pctwater",
  "nlcd_2006_forc_pct","nlcd2006_42pct_bsn","pctconif2006",
  "popdensity","popden_bsn","popden2010",
  "nlcd_2006_imperv_pct","nlcd2006_impervpct_bsn","pctimp2006",
  "road_density","roadden_bsn","rddens",
  "ndep","nadp_totaln_bsn","",
  "mercdep","mercdep_bsn","mercdep",
  )

# WARNING - unfinished business here, left off 2024Feb19
landnames2017_trim<-landnames2017[na.omit(match(landnames_df$name2017,landnames2017))]
landnames2017_trim[which(landnames2017_trim %in% landnames_df$name2017)]<-
  landnames2017_trim[which(landnames2017_trim %in% landnames_df$name2017)]

nla2017_landMets[,landnames2017[na.omit(match(landnames_df$name2017,landnames2017))]]

nla2017_landMets[,landnames2017 %in% landnames_df$name2017]

landnames2017[which(landnames2017 %in% landnames_df$name2017)]<-

daterr<-daterr %>% arrange(var)
daterr_out<-daterr
daterr_out$value<-signif(daterr_out$value,digits=3)
daterr_out<-daterr_out %>% select(site_id,uid,year,visit_no,var,value,units,depth)

daterr_profiles_out<-daterr_profiles3
daterr_profiles_out<-daterr_profiles_out  %>% select(site_id,uid,year,visit_no,var,value,units,depth)
